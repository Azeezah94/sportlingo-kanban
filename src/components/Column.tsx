...
export default function Column({ column, tasks, provided, isDraggingOver, draggingId, onUpdate, onDelete, onCreate }: Props) {
    ...
    <>
      <div
        style={{
          width: 280,
          minWidth: 280,
          background: isDraggingOver ? 'rgba(108,99,255,0.06)' : 'var(--surface)',
      ...
      >
        ...
        <div style={{ padding: '14px 16px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{ color: 'var(--text-dim)', borderRadius: 6, padding: 4, display: 'flex', alignItems: 'center', transition: 'color 0.15s' }}
        ...
        <div
          ...
          style={{ padding: '4px 10px 10px', overflowY: 'auto', flexGrow: 1, minHeight: 60 }}
        >
      ...
      {showModal && (
        <TaskModal
          ...
          onSave={(t) => { onCreate(t); setShowModal(false); }}
        />
      )}
    </>
  );
