...
import { Calendar, Trash2, Pencil, AlertCircle, ArrowUp, ArrowRight, ArrowDown } from 'lucide-react';
...
export default function TaskCard({ task, index, onUpdate, onDelete, isDragging }: Props) {
    ...
    <>
        ...
        {(provided, snapshot) => (
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
              <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', lineHeight: 1.4, flex: 1 }}>{task.title}</span>
              {hovered && (
      ...
      {showEdit && (
        <TaskModal
          ...
          onSave={(updates) => { onUpdate(task.id, updates); setShowEdit(false); }}
        />
      )}
    </>
  );
